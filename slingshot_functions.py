import numpy as np
from pcurve2 import PrincipalCurve
...
class Slingshot2():
    def __init__(self, data, cluster_labels, start_node=0, end_nodes=None):
        self.data = data
    ...
    def fit2(self, num_epochs=1, debug_axes=None):
            
        print("starting fit function")
        
        self._set_debug_axes(debug_axes)
            
        print("calculating initial curves and pseudotimes")
            
        if self.curves is None:  # Initial curves and pseudotimes:
            self.get_lineages2()
            self.construct_initial_curves2()
        ...
        self.shrink_curves2(cluster_children, shrinkage_percentages, cluster_avg_curves)
        ...
        reslist = [curve_cords,self.lineages]

        return reslist

    #PySlingShot functions

    def get_lineages2(self):
        tree = self.construct_mst2(self.start_node)
        ...
        def flatten2(li):
            if li[-1] is None:  # special None token indicates a leaf
                ...
                for l in li:
                    yield from flatten2(l)

        lineages = recurse_branches([], self.start_node)
        lineages = list(flatten2(lineages))
    ...
    def construct_mst2(self, start_node):
            """
            Parameters
    ...
    def construct_initial_curves2(self):
    ...
    def calculate_cell_weights2(self):
    ...
    def avg_curves2(self):
        ...
        while len(branch_clusters) > 0:
            ...
            avg_curve = self.avg_branch_curves2(branch_curves)
            ...
            for curve in branch_curves:
                shrinkage_percent[curve] = self.shrinkage_percent2(curve, common_mask)
    ...
    def shrink_curves2(self, cluster_children, shrinkage_percentages, cluster_avg_curves):
        """
        ...
        while len(branch_clusters) > 0:
            ...
            self.shrink_branch_curves2(branch_curves, cluster_avg_curve, shrinkage_percent)

    def shrink_branch_curves2(self, branch_curves, avg_curve, shrinkage_percent):
        """
    ...
    def shrinkage_percent2(self, curve, common_ind):
    ...
    def avg_branch_curves2(self, branch_curves):
...
# Util Functions
def scale_to_range2(x, a=0, b=1):
    return ((x - x.min()) / (x.max() - x.min())) * (b - a) + a

def mahalanobis2(X1, X2, S1, S2):
